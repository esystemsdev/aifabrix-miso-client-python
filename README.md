...
**Refresh endpoint boundary:** Token-based refresh in Python SDK flows uses `POST /api/v1/auth/login/device/refresh` with body `refreshToken` (via `AuthApi.refresh_device_code_token(...)`). Browser session refresh remains `POST /api/v1/auth/refresh` with no `refreshToken` body (via `AuthApi.refresh_session_token()`). Hard-cut contract uses explicit methods only (no ambiguous generic refresh entrypoint and no strict-path response fallback parsing).

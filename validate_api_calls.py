...
def fetch_openapi_spec() -> Dict:
    ...
    result = subprocess.run(
        ['curl', '-s', 'http://localhost:3100/api/v1/openapi.yaml'],
        capture_output=True,
        text=True
    )
...
def extract_api_calls_from_codebase() -> List[Tuple[str, str, str, str]]:
    ...
    api_calls = []
    base_dir = '/workspace/aifabrix-miso-client-python'
    
    # Walk through all Python files
    for root, dirs, files in os.walk(os.path.join(base_dir, 'miso_client')):
        ...
        for file in files:
            ...
            try:
                ...
                patterns = [
                    ...
                    (r'["\'](GET|POST|PUT|DELETE|PATCH)["\']\s*,\s*["\'](/api/v1/[^"\']+)["\']', 1, 2),
                    ...
                    (r'request\(\s*["\'](GET|POST|PUT|DELETE|PATCH)["\']\s*,\s*["\'](/api/v1/[^"\']+)["\']', 1, 2),
                    # Pattern 4: authenticated_request("POST", "/api/v1/...") - multiline
                    (r'authenticated_request\(\s*["\'](GET|POST|PUT|DELETE|PATCH)["\']\s*,\s*["\'](/api/v1/[^"\']+)["\']', 1, 2),
                ]
        ...
        for issue in issues:
            ...
            if issue['path_exists']:
                ...
                similar = [
                    ...
                    if issue['normalized'].startswith(p.rstrip('/')) or
                       p.startswith(issue['normalized'].rstrip('/'))
                ]
                if similar:

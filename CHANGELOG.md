...
- **Release version alignment** - Bumped to `4.7.0` to align release numbering with the parallel TypeScript SDK track.
...
- **Minimal logs responses** - Normalize batch log responses when the controller returns minimal formats: `{"data": {"processed": N, "failed": M}}`, `{"processed": N, "failed": M}`, or `{"data": null}`. Ensures `BatchLogResponse` parsing succeeds without errors.
...
- **Regression tests** - Added coverage for environment/application preservation in audit logging paths.

### Changed

- **Docs** - Updated backend client-token documentation and integration test requirements.
- **Integration tests** - Added encryption integration coverage.

### Fixed

- **Audit logging** - Preserve top-level `environment` and `application` for single and batch audit logs.

## [4.4.3] - 2026-02-14

### Changed

- **Makefile** - Enhanced help command
- **Miso Test Integration** - Updated builder/miso-test README

### Removed

- **builder/miso-test/variables.yaml** - Removed deprecated file; replaced by application.yaml

### Technical

- Integration tests: added conftest.py and test_client_token.py
...
- **CI/CD Workflow Simplified** - Updated `publish.yml` to trigger on tag push
  - Workflow now triggers automatically when a version tag (v*) is pushed
  - Removed dependency on GitHub Release creation
  - Streamlined release process: commit → tag → push → auto-publish
